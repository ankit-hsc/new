...
public class FragmentMetadataVisitor extends CompositeMkvElementVisitor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FragmentMetadataVisitor.class);
    ...
    private FragmentMetadataVisitor(List<MkvElementVisitor> childVisitors, MkvChildElementCollector tagCollector, MkvChildElementCollector trackCollector) {
        super(childVisitors);
    ...
    enum State {
        NEW, PRE_CLUSTER, IN_CLUSTER, POST_CLUSTER;
    }
        ...
        public void visit(MkvEndMasterElement endMasterElement) throws MkvElementVisitException {
            ...
            if (MkvTypeInfos.TAGS.equals(endMasterElement.getElementMetaData().getTypeInfo())) {
                if (log.isDebugEnabled()) {
                    log.debug("TAGS end {}, potentially updating millisbehindlatest and continuation token", endMasterElement);
                }
    ...
    private Map<Long, List<MkvElement>> getTrackEntryMap() {
        ...
        trackElements.stream().filter(e -> MkvTypeInfos.TRACKENTRY.equals(e.getElementMetaData().getTypeInfo())).forEach(e -> trackEntryElementNumberToMkvElement.put(e.getElementMetaData().getElementNumber(), new ArrayList<>()));
    ...
    private void createTrackMetadata(List<MkvElement> trackEntryPropertyLists) {
        Map<EBMLTypeInfo, MkvElement> metaDataProperties = trackEntryPropertyLists.stream().filter(e -> trackTypesForTrackMetadata.contains(e.getElementMetaData().getTypeInfo())).collect(Collectors.toMap(e -> e.getElementMetaData().getTypeInfo(), Function.identity()));
        MkvTrackMetadata mkvTrackMetadata = MkvTrackMetadata.builder().trackNumber(getUnsignedLongVal(metaDataProperties, MkvTypeInfos.TRACKNUMBER)).trackUID(getUnsignedLongValOptional(metaDataProperties, MkvTypeInfos.TRACKUID)).trackName(getStringVal(metaDataProperties, MkvTypeInfos.NAME)).codecId(getStringVal(metaDataProperties, MkvTypeInfos.CODECID)).codecName(getStringVal(metaDataProperties, MkvTypeInfos.CODECNAME)).pixelWidth(getUnsignedLongValOptional(metaDataProperties, MkvTypeInfos.PIXELWIDTH)).pixelHeight(getUnsignedLongValOptional(metaDataProperties, MkvTypeInfos.PIXELHEIGHT)).codecPrivateData(getByteBuffer(metaDataProperties, MkvTypeInfos.CODECPRIVATE)).allElementsInTrack(trackEntryPropertyLists).build();
    ...
    private static String getStringVal(Map<EBMLTypeInfo, MkvElement> metaDataProperties, EBMLTypeInfo key) {
        ...
        Validate.isTrue(EBMLTypeInfo.TYPE.STRING.equals(dataElement.getElementMetaData().getTypeInfo().getType()) || EBMLTypeInfo.TYPE.UTF_8.equals(dataElement.getElementMetaData().getTypeInfo().getType()));
