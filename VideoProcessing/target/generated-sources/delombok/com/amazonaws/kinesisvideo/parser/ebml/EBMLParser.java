...
public class EBMLParser {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(EBMLParser.class);
    ...
    public EBMLParser(EBMLTypeInfoProvider typeInfoProvider, EBMLParserCallbacks callbacks, int maxContentBytesInOnePass) {
        ...
        this.replayIdAndSizeBuffer = new ReplayIdAndSizeBuffer(EBMLUtils.EBML_ID_MAX_BYTES + EBMLUtils.EBML_SIZE_MAX_BYTES);
            ...
            while (callState.shouldContinueParsing()) {
                ...
                case SIZE_DONE: 
                    ...
                    if (currentElement.isKnownType()) {
                        ...
                        callbacks.onStartElement(currentElement.getMetadata(), currentElement.getDataSize(), replayIdAndSizeBuffer.getByteBuffer(), this::currentElementPath);
                    }
                    startReadingContentBasedOnType();
                    break;

                case CONTENT_READING: 
                    Validate.isTrue(currentElement.isKnownType(), "We should read only from elements with known types");
    ...
    public void closeParser() {
        if (!closed) {
            ...
            if (currentElement != null && currentElement.isKnownType()) {
                log.info("Closing with currentElement {} still set, invoking end element callback on it", currentElement);
                invokeOnEndElementCallback(currentElement);
                currentElement = null;
            }
            log.info("Closing with {} master elements on stack, invoking end element callback on them", masterElements.size());
    ...
    private void startReadingContentBasedOnType() {
        if (!currentElement.isKnownType()) {
            ...
            log.warn("Will skip content for element number {} with unknown id {} datasize {}", currentElement.getElementCount(), currentElement.getId(), currentElement.getDataSize());
        } else {
            if (currentElement.getTypeInfo().getType() == EBMLTypeInfo.TYPE.MASTER) {
                ...
                Validate.isTrue(!currentElement.isUnknownLength(), "A non-master element should not have unknown length");
    ...
    private class CallState implements Closeable, TrackingReplayableIdAndSizeByteSource, ParserBulkByteSource {
        ...
        @java.beans.ConstructorProperties({"byteSource"})
        @SuppressWarnings("all")
        @javax.annotation.Generated("lombok")
        public CallState(final ParserByteSource byteSource) {
            this.byteSource = byteSource;
        }

        @SuppressWarnings("all")
        @javax.annotation.Generated("lombok")
        public void setReadOffsetForReplayBuffer(final long readOffsetForReplayBuffer) {
