...
public class CountVisitor extends MkvElementVisitor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CountVisitor.class);
    ...
    public CountVisitor(Collection<EBMLTypeInfo> typesToCount) {
        ...
        this.typesToCount.stream().filter(t -> t.getType().equals(EBMLTypeInfo.TYPE.MASTER)).forEach(t -> endMasterCount.put(t, 0));
    }
    ...
    public boolean doEndAndStartMasterElementsMatch() {
        List<EBMLTypeInfo> mismatchedStartAndEnd = typeCount.entrySet().stream().filter(e -> e.getKey().getType().equals(EBMLTypeInfo.TYPE.MASTER)).filter(e -> typeCount.get(e.getKey()) != endMasterCount.get(e.getKey())).map(Map.Entry::getKey).collect(Collectors.toList());
