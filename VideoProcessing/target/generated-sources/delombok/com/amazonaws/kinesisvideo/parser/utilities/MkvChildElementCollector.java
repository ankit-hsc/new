...
public class MkvChildElementCollector extends MkvElementVisitor {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(MkvChildElementCollector.class);
    ...
    public MkvChildElementCollector(EBMLTypeInfo parentTypeInfo) {
        Validate.isTrue(parentTypeInfo.getType().equals(EBMLTypeInfo.TYPE.MASTER), "ChildElementCollectors can only collect children for master elements");
    ...
    private boolean shouldBeCollected(MkvElement mkvElement) {
        if (mkvElement.getElementPath().size() <= parentTypeInfo.getLevel()) {
            ...
            if (mkvElement.getElementMetaData().getTypeInfo().getLevel() > parentTypeInfo.getLevel()) {
                log.warn("Element {} has a path {} shorter than parent type\'s {} level but does not belong to a lower level than the parent ", mkvElement.getElementMetaData().toString(), mkvElement.getElementPath().size(), parentTypeInfo.toString());
            }
            return false;
        }
